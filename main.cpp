...
{
    // check out of bounds
    if (r <0 || r>= (int)maze.size() || c < 0 || c >= (int)maze[0].size()) {
        return false;
    }

    // Hit a wall or already visited
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // Skip invalid
        if (nr < 0 || nr >= (int)maze.size() || nc < 0 || nc >= (int)maze[0].size()) {
            continue;
        }
        ...
        // Set parent before going to neighbor
        parent_r[nr][nc] = r;
        parent_c[nr][nc] = c;
